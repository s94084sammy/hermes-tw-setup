...
官方預設 **關閉** `rich_messages`（怕難複製純文字）。台灣基線 **打開 rich，並關閉 Telegram streaming**（v0.20+ 表格才穩）。

```bash
# 自動（含 rich + streaming 關 + 進度繁中）
python3 ~/.hermes/skills/hermes-tw-setup/scripts/baseline.py apply --yes
...
# 必重啟 gateway 後才生效（依實際 unit 名稱）
systemctl --user restart hermes-gateway
# hermes -p side gateway 也要重開
```

開了之後常見坑（詳見 `TELEGRAM_RICH.md`）：

1. 沒重啟 gateway  
2. **仍開 streaming** → 先拆表成 bullet，API 有 rich 紀錄仍可能畫面失敗  
3. 表缺 `|---|` 分隔列 → 不進 rich  
4. **表前沒空一行** → 整表變直線  
5. 排程／standalone 路徑可能沒 rich，對話有  
6. 過長 fallback；舊 ptb 無 sendRichMessage  
7. 勿開 `rich_drafts`（Desktop 疊影）  
8. 主副 profile 都要同一套  
9. 表格成功須 **目視格子**（設定＋API 不夠）  

工具進度氣泡繁中（選用／apply 已含）：

```bash
python3 ~/.hermes/skills/hermes-tw-setup/bundled/tool-progress-zh/apply_patch.py
# Hermes update 後重跑；重啟 gateway
```

驗證：對 bot 送一張合法 pipe table，應見格子表不是 bullet；跑工具時氣泡應為「執行程式／搜尋檔案…」。

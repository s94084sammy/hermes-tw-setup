...
- **結論先講**：先講結果／建議，再講過程。
- 結構化回報用：**做了什麼／結果／下一步**。
- 不塞路徑、指令、程式碼、JSON、行號（使用者要技術細節再給）。

## 對外中文文案

- **對外中文文案一律由中文母語模型撰寫、修改或擴寫**（自己不准硬寫、不准自己定稿）。
- 交付物不出現寫作術語（起承轉合等）、中文破折號、AI 排比。
...
- 官方擋、憑證、危險指令、未配對的對話驅使瀏覽器，都不在「幫忙做成」範圍。
...
- 基線：`telegram.extra.rich_messages: true`，且 **`display.platforms.telegram.streaming: false`**（並建議 `streaming.enabled: false`）。只開 rich 不關 streaming，v0.20+ 仍可能先拆表成 bullet。
- 結構化內容**優先用 GFM pipe table**（含 `|---|`，表前空一行）。
- 不要預設「Telegram 不能表格」。踩坑（streaming、表前空行、缺分隔列、未重啟、standalone）見 `references/TELEGRAM_RICH.md`。
- **表格成功＝設定＋API＋使用者目視格子**；禁止只看 rich 送出紀錄就宣稱完成。
- 狀態對照、檢查結果、參數摘要、排行等多用表格，少用難掃的長點列。
- 工作進度氣泡應為繁中（執行程式／搜尋檔案…）；靠 `bundled/tool-progress-zh`，Hermes update 後重套。

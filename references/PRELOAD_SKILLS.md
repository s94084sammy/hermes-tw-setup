...
## B7. 工具進度標籤繁中（tool-progress-zh）

- 隨包：`bundled/tool-progress-zh/apply_patch.py`
- 改部署端 Hermes 的 `agent/display.py`（Running code → 執行程式 等）
- 需 `display.language: zh-TW`；Hermes update 後重跑；重啟 gateway
